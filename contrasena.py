...
def registrar(nombre, url, nombre_usuario, contrasena, descripcion):
    try:
        ...
        sentencia_sql = ''' INSERT INTO contrasena (nombre, url, nombre_usuario, contrasena, descripcion) VALUES (?, ?, ?, ?, ?) '''
...
def modificar(id, nombre, url, nombre_usuario, contrasena, descripcion):
    try:
        conexion = conectar()
        cursor = conexion.cursor()
        sentencia_sql = ''' UPDATE contrasena SET nombre = ?, url = ?, nombre_usuario = ?, contrasena = ?, descripcion = ? WHERE id = ? '''
